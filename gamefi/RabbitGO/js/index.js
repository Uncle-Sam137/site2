...
var scoreSound = new Audio('audio/score.mp3');
var woofSound = new Audio('audio/Woof.MP3');
var hedgehogSound = new Audio('audio/Stephedgehog.MP3');
var gameOverSound = new Audio('audio/over.MP3');
...
function handleWindowResize() {
  ...
  // 根据屏幕宽度调整相机位置和视野
  if (WIDTH < 768) {
    // 移动设备上调整相机位置，确保能看到完整场景
    camera.position.z = cameraPosGame * 1.2;
    camera.fov = fieldOfView * 1.1;
    if (WIDTH < 480) {
      // 小屏幕手机上进一步调整
      camera.position.z = cameraPosGame * 1.4;
      camera.fov = fieldOfView * 1.2;
    }
  } else {
    // 恢复默认设置
    camera.position.z = cameraPosGame;
...
function updateMonsterPosition(){
  ...
  // 当怪物加速追逐兔子时播放Woof音效
  if(monsterPos < 0.62 && Math.random() < 0.05 && isSoundOn) {
...
function gameOver(){
  ...
  if (isSoundOn) {
    audio.pause(); // 停止背景音乐
    gameOverSound.pause(); // 先暂停以防之前有播放
...
function toggleSound(e) {
  e.stopPropagation(); // 防止点击按钮同时触发跳跃
  isSoundOn = !isSoundOn;
